...
<table class="table table-primary table-hover text-center">
    ...
    <c:forEach items="${detail}" var="human">
        <tr>
            <td>${human.id}</td>
            <td>${human.name}</td>
            <td>${human.date}</td>
            ...
            <td>${human.address}</td>
            <td>${human.job}</td>
            <td>${human.nationality}</td>
            <td>
                <button type="button" class="btn btn-primary btn-sm" onclick="deleteId('${human.id}')"
                        data-bs-toggle="modal" data-bs-target="#confirmDeleteModal">Xóa
                </button>
            </td>
            <td><a href="/human/update?id=${human.id}" class="btn btn-primary">Chỉnh sửa</a></td>
        </tr>
    </c:forEach>
</table>
</body>
...
<div class="modal fade" id="confirmDeleteModal" tabindex="-1" aria-labelledby="confirmDeleteLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="/human/delete" method="get">
